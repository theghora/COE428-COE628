 ...
 */
unsigned int getNumCompares() { return numCompares; }

/**
 * getNumCopies returns the number of times myCopy was invoked.
 */
unsigned int getNumCopies() { return numCopies; }

/**
 * getNumSwaps returns the number of times mySwap was invoked.
 */
unsigned int getNumSwaps() { return numSwaps; }
...
int main() {
  ...
  assert(getNumSwaps() == 0);
  nTests++;
  assert(getNumCompares() == 0);
  nTests++;
  assert(getNumCopies() == 0);
  nTests++;

  /* Test compare operations */
  assert(myCompare(5, 5) == 0);
  nTests++;
  assert(myCompare(12, 34) < 0);
  nTests++;
  assert(myCompare(-5, -10) > 0);
  nTests++;
  assert(myCompare(0xffffffff, 0) < 0);
  nTests++;
  assert(getNumCompares() == 4);
  nTests++;
  ...
  assert((tmp1 == data[3]) && (tmp2 == data[2]));
  nTests++;
  mySwap(&data[2], &data[3]);
  assert((tmp1 == data[2]) && (tmp2 == data[3]));
  nTests++;
  assert(getNumSwaps() == 2);
  nTests++;
